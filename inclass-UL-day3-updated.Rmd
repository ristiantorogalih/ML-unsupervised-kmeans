...
Dimensionality reduction:
- `Feature selection` = menseleksi fitur/variable dengan cara membuang variabel2 yang sekiranya tidak berpengaruh (misal: awal 10 variabel --> kita hapus 2 var --> sisa 8 var). Contoh metode: menghapus var yg berkorelasi sgt tinggi, nearzerovar (membuang var yg variansinya ~0)
- `Feature extraction` = mengekstrak fitur/ variabel. Caranya adalah dengan membobotkan semua variabel ke dalam variabel baru (misal: awal 10 var --> 10 var baru dengan value yg berbeda --> dibuang beberapa var yang variansinya rendah/ memilih PC). metode: PCA
...
* untuk ekstrak variance (informasi)

pca$sdev^2 # variance adl kuadrat sdev (variance = eigen value)

* untuk ekstrak bobot tiap var/ kontribusi tiap variable ke PC tertentu (eigen vector)

pca$rotation

* untuk ekstrak value baru yang dihasilkan
...
Cek skala data, apakah sudah sama?
```{r}
summary(ppt)
```
...
baris 1 PC1 = -0.50181439 x baris 1 residential unit -0.18992921 x baris 1 commercial unit + ...
baris 2 PC1 = 
...
Cek perbedaan sebelum dilakukan PCA dan setelah
```{r}
ppt_z %>% 
  as.data.frame()
...
property_clean %>% 
  ...
  cbind(pc_keep) # gabungkan dengan PC1 - PC4
```

Kelebihan melakukan PCA sebelum modeling (regresi/klasifikasi/clustering):
...
Additional: kalau var sale price ingin digunakan sebagai target variabel agar gak masuk ke PCA
```{r eval = FALSE}
ppt <- property_clean %>% 
  select(-SALE.PRICE) %>% 
  select_if(is.numeric)
...
biplot(ppt_100_pca, scale = F, cex = 0.6)
```

Sumbu:
- bawah dan kiri (merepresentasikan nilai pada observasi setelah PCA)
- kanan dan atas (representasi dari kontribusi tiap var thd PC)

```{r}
cor(ppt$TOTAL.UNITS, ppt$SALE.PRICE)
```
...
- Panah merah = **loading score/eigen vector/ kontribusi var**, menunjukkan kontribusi variabel tersebut terhadap PC, atau banyaknya informasi variabel tersebut yang dirangkum oleh PC. 
...
```
Notes:
- Angka yang besar (pada sumbu) untuk representasi dari nilai observasi/titik2 setelah di PCA (bawah kiri)
- Angka yang kecil2 (pada sumbu) untuk representasi kontribusi variabel (kanan atas)
- Untuk antar sumbu apabila berhimpit dan searah (korelasi kuat positif)
- Untuk antar sumbu apabila berhimpit dan berbalik arah (korelasi kuat negatif)
- Untuk antar sumbu ~ 90 derajat (siku2) (korelasi ~0)

```{r}
ppt_100_pca$x[96:100,]
...
> Observasi 96 dan 32 merupakan outlier dan ternyata karena nilai land square feet, total units, RU, dan GSF tinggi
...
- PC1 merangkum variable: GSF, LSF, SP, CU, YB
- PC2 merangkum variable: TU, RU
...
```{r}
library(factoextra)
fviz_contrib(X = ppt_100_pca, axes = 1, choice = "var")
```
...
biplot(ppt_300_pca, #object PCA
       ...
       scale = F) # krn scr default di biplot dilakukan scaling lagi, agar bisa diinterpretasi maka scale = F
```

Mengidentifikasi outlier
- yang jauh dari kumpulan datanya
- data yang bergerombol di titik 0, artinya mereka ada di pusat datanya (nilainya mendekati rata2 dari var tsb)
...
```{r}
cor(ppt_300) # matrix korelasi
library(GGally)
ggcorr(ppt_300, label = T) # visualisasi
```



Misal kita fokus ke var total units:

```{r}
ppt_300_pca$rotation
```
...
1. Variabel yang berkontribusi tinggi ke PC1: `GROSS.SQUARE.FEET`, `LAND.SQUARE.FEET`, dan `SALE.PRICE`
2. Variabel yang berkontribusi tinggi ke PC2: `RESIDENTIAL.UNITS` dan `TOTAL.UNITS`
3. Variabel yang berkontribusi rendah pada PC1 maupun PC2: `COMMERCIAL.UNITS` dan `YEAR.BUILT`
...
1. Pasangan variabel yang berkorelasi tinggi positif (berhimpit):
  - `GROSS.SQUARE.FEET`, `LAND.SQUARE.FEET`, dan `SALE.PRICE`
  - `RESIDENTIAL.UNITS` dan `TOTAL.UNITS`

2. Pasangan variabel yang tidak berkorelasi (tegak lurus):
  - `SALE.PRICE` dan `TOTAL.UNITS`
...
1. Perhatikan cluster di mana Arizona, Colorado, Illinois, dan Texas berada. Negara bagian mana lagi yang ada dalam cluster tersebut? **New York**, dimana cluster tersebut tinggi untuk tingkat Rape

2. Di antara Ohio, Louisiana, North Dakota, dan Colorado, manakah yang memiliki:

- Tingkat `Murder` tertinggi: **Louisina**
- Tingkat `Assault` tertinggi: **Louisina**
...
Notes:
ncp = 8 karena jumlah variabel numeric pada loan_clean adalah 8 (jumlah quantivar), tujuannya adalah kita bisa akses seluruh hasil PC
...
cek data-data yang termasuk outlier
```{r}
loan_clean[c(1228,1146,368,749,512),]
```

cek output dr PCA (sama dengan obj_pca$x pada base R)
```{r}
...
utk cek nilai eigen vectors (besarnya kontribusi thd PC tertentu)
```{r}
loan_pca$var$coord
```
...
- PC1 paling banyak merangkum variable: log_inc, annual_inc
- PC2 merangkum variable: dti, int_rate
- Variabel yang saling berkorelasi:
  - installment dan revolbal
  - annual inc log inc
  - inq_las_12m juga korelasi dengan revol bal #korelasi tidak sesuai dengan arah garis panah karena pada plot baru ditampilkan ~47% informasi
...
Notes:
Umumnya yang digunakan adalah 70-90% informasi
...
- Within Sum of Squares (`$withinss`): jumlah jarak kuadrat dari tiap observasi ke centroid tiap cluster. kecil lebih baik supaya karakterisitk tiap observasi mirip
- Between Sum of Squares (`$betweenss`): jumlah jarak kuadrat terbobot dari tiap centroid tiap cluster ke centroid global. Dibobotkan berdasarkan banyaknya observasi pada cluster.
- Total Sum of Squares (`$totss`): jumlah jarak kuadrat dari tiap observasi ke rata-rata global.
- tot.withinss: jumlah jarak kuadrat dari tiap observasi ke centroid cluster.
...
Pilih nilai $k$ dimana ketika $k$ ditambah, penurunan Total WSS tidak terlalu drastis (atau dapat dikatakan melandai). Pilih di titik siku elbownya yaitu 5-6
...
- Cluster 1: body cukup tinggi, sweetness cukup tinggi, medicinal kecil, honey, winey, nutty tinggi
- Cluster 2: 
- Cluster 3: 
- Cluster 4: 
- Cluster 5: 
